...
daedalus はデフォルトで **apply 前に tfsec スキャンを必須化**する（CRITICAL/HIGH の検出が
ゼロになるまで apply をブロック）。未導入の場合は警告して自動無効化されるが、導入を推奨。

```bash
# macOS
brew install tfsec
# Linux: GitHub Releases からバイナリを取得して PATH に置く
...
スキャンを明示的に無効化したい場合は `--no-security-scan`（CLI）か GUI のチェックボックスを外す。
...
| apply が `security gate` で denied | 仕様。tfsec の CRITICAL/HIGH を修正してクリーンなスキャンが通れば解禁される。スキップは `--no-security-scan` |
| `tfsec not found` の警告が出る | tfsec を導入する（§1）。導入しない場合スキャン必須化は自動で無効化される |
