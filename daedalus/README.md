...
```bash
daedalus serve            # → http://127.0.0.1:8420
```
...
```bash
# dry-run（デフォルト）
daedalus run examples/spec.example.yaml --workspace ./workspace

# 承認モード（apply のたびにターミナルで y/N）
daedalus run <spec> --workspace <dir> --mode approval

# 完全自動運転（サンドボックス向け）
daedalus run <spec> --workspace <dir> --mode auto

# GitHub 連携（spec の github: 設定を使用）
daedalus pull <spec> --workspace <dir>
daedalus push <spec> --workspace <dir> -m "update infra"
```
