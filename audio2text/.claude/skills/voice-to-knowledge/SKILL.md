...
description: workspace/inbox/ にある音声ファイルを文字起こし→要約・分類してナレッジファイル化→処理済みファイルをアーカイブに移動する一連のワークフローを実行する。会議録音やインタビュー音声から知識ベースを構築したい時に使う。
---

# voice-to-knowledge

音声ファイルから知識ベースを構築するワークフロースキル。

## 概要

`workspace/inbox/` に置かれた音声ファイルを順番に処理し、最終的にカテゴリ分類されたナレッジファイルを `workspace/knowledge/<category>/` に生成する。処理済みファイルは `workspace/archive/` に移動する。
...
[workspace/transcripts/foo.md]   ← Whisper 出力
    │ 3-A. ソースノート生成 (1音声 = 1ファイル、サマリー兼インデックス)
    ▼
[workspace/knowledge/sources/<category>/YYYY-MM-DD_<slug>.md]
    │ 3-B. アトミックノート抽出 (1テーマ = 1ファイル、再利用可能な原子)
    ▼
[workspace/knowledge/{decisions,tasks,facts,ideas,insights,questions,concepts}/YYYY-MM-DD_*.md]
    │ 3-C. ソースノートに extracted_notes リンクを記入
    ▼
[ソースノート: 各 atomic ノートへのリンク集として機能]
    │ 4. 元ファイルをアーカイブ
    ▼
...
│   └── general/
│
├── decisions/                     ← 第2層: 抽出された原子ノート（横断検索の単位）
├── tasks/
├── facts/
├── ideas/
├── insights/
...
**ソースノート**: 「この会議全体は何だったか」を保ち、抽出されたアトミックノートへのリンクを集約するインデックス。
**アトミックノート**: 1テーマ = 1ファイル。複数のソースから参照されうる再利用可能な原子。横断検索の主役。
...
| カスタマイズ対象 | 場所 |
|---|---|
| 文字起こしのモデル/前処理/後処理/Whisper プロンプト | `presets.json` |
| ソースノートの構造（カテゴリ別） | `templates/<category>.md` |
| アトミックノートの構造（type 別） | `templates/atomic/<type>.md` |
| カテゴリ分類のルール | この `SKILL.md` の Step 6-1 セクション |
| アトミック抽出のルール | この `SKILL.md` の Step 7 セクション |
...
### Step 6: ソースノートを生成（第1層）

「この音声全体は何だったか」を表す **1音声 = 1ファイル** のサマリーを作成する。
...
の3点を厳守する。テンプレートに無いセクションを勝手に追加したり、構造を変えたりしない。
...
`workspace/knowledge/sources/<category>/<filename>` 。カテゴリディレクトリが無ければ作成する。

この時点では `extracted_notes` フィールドと「抽出されたアトミックノート」セクションは空欄のまま記入。Step 7 で実際の抽出後にリンクを記入する。

### Step 7: アトミックノート抽出（第2層）

ソースノートの内容を読み直し、**再利用可能な知識の原子** を type 別に切り出して保存する。

#### Step 7-1: 抽出ルール（粒度）

- **1ノート = 1テーマ**: 「Q3リリース」と「予算削減」が同じ会議で決まったら、ノートも2枚に分ける
- **タイトルが完結文か**: "Q3リリース日を8/15に確定" のように、タイトルだけで意味が分かる
- **すべて`source:` を持つ**: frontmatter に元のソースノート（`sources/<category>/<file>.md`）へのリンクを必須記入
- **空であれば作らない**: 該当する原子が無ければ、その type のディレクトリには何も生成しない

#### Step 7-2: type 別の抽出基準

各 type について、対応する atomic テンプレート (`templates/atomic/<type>.md`) を Read してから抽出する。

| type | 何を切り出すか |
|---|---|
| `decision` | 「やる/やらない」「Aに合意した」のように意思決定が読み取れるもの |
| `task` | 「Xする」「Yに連絡する」のように行動可能な表現 |
| `fact` | 自分の意見ではない、客観的な事実・知識・数値・仕様 |
| `idea` | 「○○できそう」「△△を試したい」のような採否未定の発想 |
| `insight` | 「○○だと思った」「○○な傾向がある」のような主観的気付き・教訓 |
| `question` | 「○○ってどうなんだろう」のような未解決の問い |
| `concept` | 会話中に定義・説明された用語（単に名前が出ただけは不要） |

#### Step 7-3: ファイル名

`<type>/YYYY-MM-DD_<slug>.md`

- `YYYY-MM-DD`: 録音日
- `<slug>`: タイトルから生成。日本語可・英数字とハイフン。30文字以内
- 同日重複時: 末尾に `-2` `-3`
...
#### Step 7-4: アトミックノート本文の生成

各 atomic ノートは、対応する `templates/atomic/<type>.md` の **「出力フォーマット」をそのまま採用** して生成する。

frontmatter には以下を必ず記入:
- `type`, `title`, `created_at`, `recorded_at`
- `source: sources/<category>/<source-file>.md` ← ソースノートへのバックリンク必須
- `people`, `project`, `tags` （該当する範囲で）
- type 固有のフィールド（`status`, `due`, `assignee`, `confidence` など）

#### Step 7-5: 既存アトミックノートとの重複チェック

`concept` だけは特に注意。同じ用語のノートが既に `concepts/` に存在するか確認し、ある場合は **新規作成せず既存を更新** する（aliases や説明を追記）。

その他の type は基本的に新規作成で良い（同じ決定が複数音声に出てきた場合、それぞれ別の文脈での再確認なので別ノートでも有用）。

#### Step 7-6: ソースノートへのリンク記入

抽出が終わったら、Step 6 で生成したソースノートを編集する:

1. frontmatter の `extracted_notes` 配下にファイルパスを追加
2. 本文の「## 抽出されたアトミックノート」セクションのプレースホルダーを実際のリンクに置き換え

例（business-meeting のソースノート）:

```markdown
extracted_notes:
  decisions:
    - decisions/2026-05-08_q3-deadline.md
    - decisions/2026-05-08_budget-cut.md
  tasks:
    - tasks/2026-05-08_update-readme.md
  facts:
    - facts/2026-05-08_jwt-rotation.md
  ideas:
    - ideas/2026-05-08_cache-redis.md
  insights: []
  questions: []
  concepts: []
```
...
### 決定事項
- [[decisions/2026-05-08_q3-deadline]] — Q3リリース日を8/15に確定
- [[decisions/2026-05-08_budget-cut]] — Q3予算を10%削減

### タスク
- [[tasks/2026-05-08_update-readme]] — README を更新する（担当: 山田、期日: 5/15）

### 知識・事実
- [[facts/2026-05-08_jwt-rotation]] — JWT のローテーション方針

### アイデア
- [[ideas/2026-05-08_cache-redis]] — Redis でキャッシュ無効化を非同期化する
```

該当する type が空なら、そのサブセクション自体を削除しても良い（テンプレートのプレースホルダーを残さない）。
...
- workspace/knowledge/sources/business-meeting/2026-05-08_foobar-mtg.md
  (preset: business-meeting)
- workspace/knowledge/sources/private-chat/2026-05-08_yyy.md
  (preset: private-chat)
