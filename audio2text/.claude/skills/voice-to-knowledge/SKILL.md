...
生成されたソースファイルは `omni-archive` (`/Users/satoshi-onga/Documents/portfolio/omni-archive`) の inbox に **手動で** 取り込む運用とする。本スキルは生成までを担当し、その先のコピー・分割は受け持たない。
...
生成されたソースノートを omni-archive (/Users/satoshi-onga/Documents/portfolio/omni-archive) の inbox に取り込む場合は、人間がコピーする。
```
